...
更新：2026-07-28（**扩标准类目词汇表**：默认 `allowed_categories` 由 5 项扩至 23 项（生活必需/日常开销/生活品质/大额与保障[含 房产/车辆]/投资理财/兜底），保留受保护的「合理享受」；投资理财组 `投资/理财/基金/股票/黄金` 仅作资金去向标签，`invest_ratio` 投资机制不受影响。仍为推荐清单、非硬约束（judge 不强制成员校验，选项 B 待定）。专用开关 `--add-category/--remove-category` 不变；测试 244 → 247（新增 test_models 锁定默认值 + 夹具改 园艺/健身））
