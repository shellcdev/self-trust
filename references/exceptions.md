# exceptions — 申诉/覆写/护栏修改冷路径（§5.2 / §5.4）

> 适用：驳回后申诉、连续 3 次触发人工覆写、修改契约核心护栏字段。低频，别陪跑高频审批。

## 1. 申诉流程（§5.2）[待实施]

1. 驳回后用户说「记账申诉 <理由>」→ 引擎按 §4.4 **同一判定逻辑**重审（不换标准）；
2. 仍不达标 → 维持驳回，`appeal_count += 1`（按 request_id 关联，换申请自动归零）；
3. `appeal_count >= 3` → 开放**一次性**人工复核覆写入口，提示「请作为配置者亲自裁决」。

## 2. 人工覆写（§5.2）[待实施]

- 覆写前**必须确认知悉目标延后时长**（引擎 F5/F7 输出，原样引用）；
...
唯一整文件重建入口：二次确认（「确认重置」）→ 仅重写 contract.json，**audit/ 全部保留**，`override_log` 记 `event=contract_reset`（含旧契约哈希）。
