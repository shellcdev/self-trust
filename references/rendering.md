...
============================================
{4.1 警告行}
· 场景A {label} ¥{amount} → 批准（无冷静期）[· 目标延后约 {n} 个月（简化口径）]
· 场景B {label} ¥{amount} → 批准（触发 {days} 天冷静期）[· 目标延后约 {n} 个月]
· 场景C {label} ¥{amount} → 驳回（冷却）[· 目标延后约 {n} 个月]
...
- 无冷静期（A-1/A-3）不输出冷静期行；A-2（有冷却）在判定行后追加「冷静期 {days} 天，到期 {expire_at}（编号 {request_id}）」即可。**若 `expire_at` / `request_id` 为 null，按 §0.5 字段缺失降级规则处理**——降级为「冷静期 {days} 天，到期终裁（§5.1）」且**不显编号**（编号属内部队列标识，用户无需见），严禁把 `null`/字段名/调试语暴露给用户。
